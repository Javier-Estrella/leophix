...
class Customizer_Repeater extends WP_Customize_Control {
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		$this->add_field_label = esc_html__( 'Add new field', 'grasim-shop' );
		if ( ! empty( $args['add_field_label'] ) ) {
			$this->add_field_label = $args['add_field_label'];
		}

		$this->boxtitle = esc_html__( 'Customizer Repeater', 'grasim-shop' );
		...
		if ( file_exists( IFT_PLUGIN_DIR_PATH . 'includes/free/customizer-repeater/inc/icons.php' ) ) {
			$this->customizer_icon_container =  'includes/free/customizer-repeater/inc/icons';
		}
	...
	public function enqueue() {
		...
		wp_enqueue_style( 'customizer-repeater-admin-stylesheet', IFT_PLUGIN_DIR . '/includes/free/customizer-repeater/css/admin-style.css', array(), 999 );

		wp_enqueue_style( 'wp-color-picker' );

		wp_enqueue_script( 'customizer-repeater-script', IFT_PLUGIN_DIR . '/includes/free/customizer-repeater/js/customizer_repeater.js', array('jquery', 'jquery-ui-draggable', 'wp-color-picker' ), 999, true  );

		wp_enqueue_script( 'customizer-repeater-fontawesome-iconpicker', IFT_PLUGIN_DIR . '/includes/free/customizer-repeater/js/fontawesome-iconpicker.js', array( 'jquery' ), 999, true );

		wp_enqueue_style( 'customizer-repeater-fontawesome-iconpicker-script', IFT_PLUGIN_DIR . '/includes/free/customizer-repeater/css/fontawesome-iconpicker.min.css', array(), 999 );
	}
                    ...
                    <div class="customizer-repeater-box-content-hidden">
						...
						if($this->customizer_repeater_color_control == true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Color','grasim-shop' ), $this->id, 'customizer_repeater_color_control' ),
						...
						if($this->customizer_repeater_color2_control == true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Color','grasim-shop' ), $this->id, 'customizer_repeater_color2_control' ),
						...
						if($this->customizer_repeater_title_control==true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Title','grasim-shop' ), $this->id, 'customizer_repeater_title_control' ),
						...
						if($this->customizer_repeater_subtitle_control==true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Subtitle','grasim-shop' ), $this->id, 'customizer_repeater_subtitle_control' ),
						...
						if($this->customizer_repeater_text_control==true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Text','grasim-shop' ), $this->id, 'customizer_repeater_text_control' ),
						...
						if($this->customizer_repeater_link_control){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Link','grasim-shop' ), $this->id, 'customizer_repeater_link_control' ),
						...
						if($this->customizer_repeater_text2_control==true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Button Text','grasim-shop' ), $this->id, 'customizer_repeater_text2_control' ),
						...
						if($this->customizer_repeater_link2_control){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Link','grasim-shop' ), $this->id, 'customizer_repeater_link2_control' ),
						...
						if($this->customizer_repeater_twitter_control){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Twitter Link','grasim-shop' ), $this->id, 'customizer_repeater_twitter_control' ),
						...
						if($this->customizer_repeater_facebook_control){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Facebook Link','grasim-shop' ), $this->id, 'customizer_repeater_facebook_control' ),
						...
						if($this->customizer_repeater_instagram_control){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Instagram Link','grasim-shop' ), $this->id, 'customizer_repeater_instagram_control' ),
						...
						if($this->customizer_repeater_linkedin_control){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Linkedin Link','grasim-shop' ), $this->id, 'customizer_repeater_linkedin_control' ),
						...
						if($this->customizer_repeater_shortcode_control==true){
							$this->input_control(array(
								'label' => apply_filters('repeater_input_labels_filter', esc_html__( 'Shortcode','grasim-shop' ), $this->id, 'customizer_repeater_shortcode_control' ),
                        ...
                        <button type="button" class="social-repeater-general-control-remove-field" <?php if ( $it == 0 ) {
							...
							<?php esc_html_e( 'Delete field', 'grasim-shop' ); ?>
                        </button>
	...
	private function icon_picker_control($value = '', $show = ''){
        ...
        <div class="social-repeater-general-control-icon" <?php if( $show === 'customizer_repeater_image' || $show === 'customizer_repeater_none' ) { echo 'style="display:none;"'; } ?>>
            <span class="customize-control-title">
                <?php esc_html_e('Icon','grasim-shop'); ?>
            </span>
            <span class="description customize-control-description">
                <?php
                echo sprintf(
	                esc_html__( 'Note: Some icons may not be displayed here. You can see the full list of icons at %1$s.', 'grasim-shop' ),
	                sprintf( '<a href="http://fontawesome.io/icons/" rel="nofollow">%s</a>', esc_html__( 'http://fontawesome.io/icons/', 'grasim-shop' ) )
                ); ?>
            </span>
	...
	private function image_control($value = '', $show = ''){ ?>
        <div class="customizer-repeater-image-control" <?php if( $show === 'customizer_repeater_icon' || $show === 'customizer_repeater_none' ) { echo 'style="display:none;"'; } ?>>
            <span class="customize-control-title">
                <?php esc_html_e('Image','grasim-shop')?>
            </span>
            <input type="text" class="widefat custom-media-url" value="<?php echo esc_attr( $value ); ?>">
            <input type="button" class="button button-secondary customizer-repeater-custom-media-button" value="<?php esc_attr_e( 'Upload Image','grasim-shop' ); ?>" />
        </div>
		<?php
	}
	...
	private function icon_type_choice($value='customizer_repeater_icon'){ ?>
        <span class="customize-control-title">
            <?php esc_html_e('Image type','grasim-shop');?>
        </span>
        <select class="customizer-repeater-image-choice">
            <option value="customizer_repeater_icon" <?php selected($value,'customizer_repeater_icon');?>><?php esc_html_e('Icon','grasim-shop'); ?></option>
            <option value="customizer_repeater_image" <?php selected($value,'customizer_repeater_image');?>><?php esc_html_e('Image','grasim-shop'); ?></option>
            <option value="customizer_repeater_none" <?php selected($value,'customizer_repeater_none');?>><?php esc_html_e('None','grasim-shop'); ?></option>
        </select>
		<?php
	}
	...
	private function repeater_control($value = ''){
		...
		$show_del        = 0; ?>
        <span class="customize-control-title"><?php esc_html_e( 'Social icons', 'grasim-shop' ); ?></span>
		<?php
            ...
            <div class="customizer-repeater-social-repeater">
                <div class="customizer-repeater-social-repeater-container">
                    ...
                    <input type="text" class="customizer-repeater-social-repeater-link"
                           placeholder="<?php esc_attr_e( 'Link', 'grasim-shop' ); ?>">
                    <input type="hidden" class="customizer-repeater-social-repeater-id" value="">
                    <button class="social-repeater-remove-social-item" style="display:none">
						<?php esc_html_e( 'Remove Icon', 'grasim-shop' ); ?>
                    </button>
                </div>
                <input type="hidden" id="social-repeater-socials-repeater-colector" class="social-repeater-socials-repeater-colector" value=""/>
            </div>
            <button class="social-repeater-add-social-item button-secondary"><?php esc_html_e( 'Add Icon', 'grasim-shop' ); ?></button>
			<?php
		} else { ?>
            <div class="customizer-repeater-social-repeater">
				<?php
                    ...
                    <div class="customizer-repeater-social-repeater-container">
                        ...
                        <button class="social-repeater-remove-social-item"
                                style="<?php if ( $show_del == 1 ) {
							        echo "display:none";
						        } ?>"><?php esc_html_e( 'Remove Icon', 'grasim-shop' ); ?></button>
                    </div>
					<?php
				} ?>
